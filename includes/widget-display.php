<?php
	...
	if($option['gmap']) {
		...
		//$map_location.="<div class='fsmayortitle'>My Last Checkin Map</div>";
		/*$map_location.='<script src="http://maps.google.com/maps?file=api&amp;v=2&amp;key='.$option['gmap_key'].'" type="text/javascript"></script><script type="text/javascript"> window.onload=init;function init(){initialize('.$location[0]['venue'][0]['geolat'].','.$location[0]['venue'][0]['geolong'].');}window.onunload=GUnload;function initialize(lat,lng) {if (GBrowserIsCompatible()) {var map = new GMap2(document.getElementById("map_canvas"));var center = new GLatLng(lat, lng);map.setCenter(center, 13);var marker = new GMarker(center);map.addOverlay(marker);}}</script><div id="map_canvas" style="width: 260px; height: 253px"></div><br>';*/
		$map_location.="<a style='text-decoration:none;border:none;' target='_blank' href=http://foursquare.com/venue/".$lid."><div id='map_canvas' style='width: 300px; height: 200px'><img style='border:none;'  src='http://maps.google.com/maps/api/staticmap?zoom=14&size=253x250&maptype=roadmap&markers=color:red|color:red|label:C|".$geolat.",".$geolong."&sensor=false' /></div></a><div style='height:60px'>&nbsp;</div>";
	}
